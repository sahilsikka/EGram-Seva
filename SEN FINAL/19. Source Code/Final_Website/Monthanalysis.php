<?php
	...
	if(mysql_select_db("$dbname",$link))
       	{
          print"Month-wise Analysis of total no of Users"; //see if font can be changed?
      	}
	$sql="SELECT MONTHNAME(reg_mod_date) AS regmonth,COUNT(mobile) AS Users from $tablename GROUP BY regmonth  ORDER BY MONTH(reg_mod_date) "; 
	$result = mysql_query($sql); 
	$graphtitle='BarChart';//Graph Title
	$xname='Month'; //X-axis Name
	...
	$maxvalue="select COUNT(mobile) from $tablename GROUP BY EXTRACT(MONTH FROM reg_mod_date) ORDER BY COUNT(mobile) DESC LIMIT 0,1"; 
...
//header('Content-type: image/png');
	...
	echo "<div style='border:1px solid #d8d8d8;width:$img_width'><img src='barchart1.jpg'></div>";
	?>
